...
import static gtr.common.Sounds.JET_ENGINE;
...
public class ArmorUtils {
    ...
    public static class ModularHUD {
        ...
        private Pair<Integer, Integer> getStringCoord(int index) {
            ...
            switch (ConfigHolder.hudLocation) {
                case 1:
                    posX = 1 + ConfigHolder.hudOffsetX;
                    posY = 1 + ConfigHolder.hudOffsetY + (fontHeight * index);
                    break;
                case 2:
                    posX = windowWidth - (1 + ConfigHolder.hudOffsetX) - stringWidth;
                    posY = 1 + ConfigHolder.hudOffsetY + (fontHeight * index);
                    break;
                case 3:
                    posX = 1 + ConfigHolder.hudOffsetX;
                    posY = windowHeight - fontHeight * (stringAmount - index) - 1 - ConfigHolder.hudOffsetY;
                    break;
                case 4:
                    posX = windowWidth - (1 + ConfigHolder.hudOffsetX) - stringWidth;
                    posY = windowHeight - fontHeight * (stringAmount - index) - 1 - ConfigHolder.hudOffsetY;
