...
import gtr.api.worldgen.populator.FluidSpringPopulator;
...
import static gtr.api.GTValues.*;
...
public class GTOreInfo implements IRecipeWrapper {
    ...
    public void addTooltip(int slotIndex, boolean input, Object ingredient, List<String> tooltip) {
        ...
        //Surface Indicator slot
            ...
            if(veinPopulator instanceof SurfaceRockPopulator) {
                tooltip.add(I18n.format("gtr.jei.ore.surface_rock_1"));
                tooltip.add(I18n.format("gtr.jei.ore.surface_rock_2"));
            }
        }
        else {
    ...
    public List<String> createBiomeTooltip() {
        ...
        for(Map.Entry<Biome, Integer> entry : modifiedBiomeMap.entrySet()) {
            ...
            if(!(entry.getValue() == weight)) {
                //Cannot Spawn
                if(entry.getValue() <= 0) {
                    tooltip.add(I18n.format("gtr.jei.ore.biome_weighting_no_spawn", entry.getKey().getBiomeName()));
                }
                else {
                    tooltip.add(I18n.format("gtr.jei.ore.biome_weighting", entry.getKey().getBiomeName(), entry.getValue()));
                }
            }
        }
