...
import gtr.api.render.scene.SceneRenderCallback;
import gtr.api.render.scene.WorldSceneRenderer;
...
public class MultiblockInfoRecipeWrapper implements IRecipeWrapper, SceneRenderCallback {
    ...
    public void setRecipeLayout(RecipeLayout layout, IGuiHelper guiHelper) {
        ...
        for (int i = 0; i < MAX_PARTS; ++i) itemStackGroup.init(i, true, 18*i-180*(i/10), border.getHeight()-36+18*(i/10));
        this.panX = 0.0f;
        this.panY = 0.0f;
    ...
    private static void gatherBlockDrops(World world, Map<BlockPos, BlockInfo> blocks, Set<ItemStackKey> drops, Map<ItemStackKey, PartInfo> partsMap) {        NonNullList<ItemStack> dropsList = NonNullList.create();
